...
''' 
改进版的main
在进行BFS选择时，依据点与当前结点(非原始点，会不停变)之间共同打分的个数进行排序
每次选择都是
'''
...
def	main():
	...
	writer=open(workPath+'trust200_undirect'+version+'.txt','w')
	neighBourNum=200
...
def SpecialBFS(fNode,graph,neighBourNum,coreNodeList,totalNodeList,nodeMapNumDict,userItemMapDict):
	...
	while len(queueList)>0:
		...
		neighbourList = TuneOrderByCommonRating(currentNodeIndex,neighbourSet,userItemMapDict,nodeMapNumDict,totalNodeList,coreNodeList)
