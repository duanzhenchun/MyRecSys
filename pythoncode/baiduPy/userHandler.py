...
def main():
	...
	rtActiveUserList,itemNumList=GetRatingActiveUser(ratingUserSet,userRatingDataMapDict,1,3000000)
	...
	GetRatingCountDistribution(finalUserSet,userRatingDataMapDict)
	
	print 'get final user and start to write data to txt...'
	WriteFinalUserDataToTxt(finalUserSet)
...
def WriteFinalUserDataToTxt(finalUserSet):
	codePath=sys.path[0]
	s=codePath.split('\\')
	workPath=s[0]+'\\'+s[1]+'\\'+s[2]+'\\data\\baidu\\commondata\\'  #f:\project\somproject
	writer=open(workPath+'rawCoreUserID.txt','w')
	str=''
