...
def main():
	...
	codePath=sys.path[0]
	s=codePath.split('\\')
	workPath1=s[0]+'\\'+s[1]+'\\'+s[2]+'\\data\\flixster\\'  #f:\project\somproject
	workPath2=s[0]+'\\'+s[1]+'\\'+s[2]+'\\data\\flixster\\commondata\\'  #f:\project\somproject
	workPath3=s[0]+'\\'+s[1]+'\\'+s[2]+'\\data\\flixster\\svddata\\'  
	...
	svdModelName=workPath3+'socialMF_rm1_wm0002_'+str(n_factors)+'.mat'
...
def ALSTrain(R,S,n_factors,n_iterations,lambda_,epsilon_,beta_,rm,wm):
	m,n=R.shape
	Q =  np.sqrt(5 - rm) * np.random.rand(m, n_factors)   # m x f   user factor matrix
	P =  np.sqrt(5 - rm) * np.random.rand(n, n_factors)   # n x f   item factor matrix
	W = R>0
	W[W == True] = 1
	W[W == False] = wm
