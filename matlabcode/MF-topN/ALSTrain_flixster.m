function [Q,P] = ALSTrain_flixster(R, trainSet, uniqUserData ,uniqItemData, n_factors,n_iterations,lambda,rm,wm,costTol)
...
%            b = zeros(n_factors,1);   
%             Q(u,:) = (A\b)';
             Q(u,:) =  zeros(1, n_factors);
       else
           A = uFixedMatrix - wm * P(iList, :)' * P(iList, :) + P(iList, :)' * diag(WuiList) * P(iList, :)+ lambda * eye(n_factors);
           b = P(iList,:)' * diag(WuiList) * (Ru(iList)' - rm);     
           Q(u,:) = (A\b)';
      end
...
%         uList = find(R(:, i) > 0);
    ...
    cost = GetCost_flixster(R, Q, P, wm, rm, trainSet,uniqUserData ,uniqItemData, lambda);
