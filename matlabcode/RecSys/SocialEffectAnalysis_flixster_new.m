...
oldRecInfoFileName = sprintf('..\\..\\..\\data\\flixster\\somdata\\100oldRecInfo%d.mat',version);
newRecInfoFileName = sprintf('..\\..\\..\\data\\flixster\\somdata\\100newRecInfo%d.mat',version);
...
for i = 1: userCount
    ...
    newRecList = newRecInfoCell{7};
    oldRecList = oldRecInfoCell{7};
   ...
   elseif socialItrHitLen == 0 && isempty(diffHitList)
       recListDiff = setdiff(newRecList,oldRecList);
       recInterestSet = itemClassIndex(recListDiff);
       haha = intersect(recInterestSet,socialInrerestSet );  % 看看与social时的兴趣重合的情况
       hahaLen = length(haha);
       if hahaLen >0 && hahaLen == length(recInterestSet)
           effectType = 1;
           effectType1 = effectType1 + 1;
           allUserChangedEvaluation(i, :) = [testUser,changedPrecision,changedRecall,0,0,0,0,level];
       elseif hahaLen==0
           ...
           effectType2 = effectType2 + 1;
           allUserChangedEvaluation(i, :) = [testUser,0,0,changedPrecision,changedRecall,0,0,level];
       elseif hahaLen >0 && hahaLen <= length(recInterestSet)
           effectType = 3;
           effectType3 = effectType3 + 1;
           allUserChangedEvaluation(i, :) = [testUser,0,0,0,0,changedPrecision,changedRecall,level];
       end
...
for i=1:levelNum
    ...
    SocialEffectFileName = sprintf('..\\..\\..\\result\\flixster\\som\\cmim\\new_flixster_SocialEffect_level%d.txt',i);
