function interestRecList=GetRecListByInterestCircle(testUserID,userInterestCircle,itemInterestCircle,userRatingMatrix,recThreshold,userAvgRating)
% 利用用户的单个兴趣圈来产生推荐
...
testUserRatingMatrix(testUserRatingMatrix>0)=1;
...
interestAccep(:,2)=interestAccep(:,2)/(length(itemInterestCircle));
...
% 所有兴趣用户的平均打分
% irAvgMatrix=repmat(userAvgRating(simiWeight(:,1)),1,length(itemInterestCircle));
% 0 index
...
% itemAvgWeightRating(:,2)=itemAvgWeightRating(:,2)+testUserAvgRating;
