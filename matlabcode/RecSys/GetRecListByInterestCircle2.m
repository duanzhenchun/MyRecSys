...
Jaccard=zeros(itrUserCount,2);
Jaccard(:,1)=userInterestCircle(:,1);
MSD = zeros(itrUserCount,2);
MSD(:,1) = userInterestCircle(:,1);
...
% 获取各个interset用户打分信息
urMatrix=userRatingMatrix(Jaccard(:,1),itemInterestCircle);
% 获取testuser信息
...
% diffMatrix 评分之差
diffMatrix = (urMatrix - testUserRatingMatrix)/5;
...
commonBase = sum(commonMatrix,2);
unionBase=sum(unionMatrix,2);

diffMatrix = diffMatrix.*commonMatrix;
diffMatrix = diffMatrix.^2;
MSD(:,2) = sum(diffMatrix, 2);

% MSD
MSD(:,2) = MSD(:,2)./(commonBase+1);

% jaccard 相似度
% 防止出现0，全部加上1
Jaccard(:,2)=(commonBase+1)./(unionBase+1);
...
simiWeight(:,2)=userInterestCircle(:,2).*Jaccard(:,2).*(1-MSD(:,2));
