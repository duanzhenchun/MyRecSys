...
for m =1 :10
    topK = topKList(m);
    finalResultFileName=sprintf('..\\..\\..\\result\\baidu\\som\\cmim\\final_baidu_cmim_prf_top%d.txt',topK);
    fid = fopen(finalResultFileName,'a');
    fprintf(fid,'%f\t%f\t%f\r\n',avgPrecision(m),avgRecall(m),avgF1(m));
    fclose(fid);
end

allUserRecInfoFile = sprintf('..\\..\\..\\result\\baidu\\som\\cmim\\allUserRecInfo%d.mat',version);
