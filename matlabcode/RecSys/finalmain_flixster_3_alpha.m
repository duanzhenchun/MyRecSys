...
for m = 1:5
    topK = topKList(m);
    finalResultFileName=sprintf('..\\..\\..\\result\\flixster\\som\\cmim\\final_flixster_cmim_prf_top%d_alpha%.1f.txt',topK,alpha);
    fid = fopen(finalResultFileName,'a');
    fprintf(fid,'%f\t%f\t%f\r\n',avgPrecision(m),avgRecall(m),avgF1(m));
    fclose(fid);
end

finalDiversityFileName=sprintf('..\\..\\..\\result\\flixster\\som\\cmim\\final_flixster_cmim_diversity_alpha%.1f.txt',alpha);
finalColdUserResultFileName = sprintf('..\\..\\..\\result\\flixster\\som\\cmim\\final_flixster_cmim_colduser_alpha%.1f.txt',alpha);
