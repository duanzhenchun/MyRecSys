...
%     version = 1;
    ...
    saveUserSimiFileName=sprintf('..\\..\\..\\data\\baidu\\cfulfdata\\MF_userSimiMatrix%d.mat',version);
    coreUserFileName = '..\\..\\..\\data\\baidu\\commondata\\coreUserID.txt';
    ...
    load(featureMatrixFileName,'Q');
    
    userFeatureMatrix = Q;
    
%     % 获得所有用户的平均打分
%     userAvgRating=GetAllUserAvgRating(userFeatureMatrix);
...
%         disp(i)
        ...
        for j=i+1:userCount
            ...
            userIVector=userFeatureMatrix(i,:);
            userJVector=userFeatureMatrix(j,:);
...
%             end
            ...
            userIAvgRating=mean(userIVector);
            userJAvgRating=mean(userJVector);      

            % 求pearson相似度
            % for p in comset ,
            % sum[(ri,p-avgi)*(rj,p-avgj)]/(sum(ri,p-avgi)^2) ^(1/2)+(sum(rj,p-avgj)^2) ^(1/2)

            userIBias = userIVector-userIAvgRating;        
            userJBias = userJVector-userJAvgRating;    
